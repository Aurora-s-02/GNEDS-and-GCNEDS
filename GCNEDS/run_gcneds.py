...
from gcneds_distilbert import gcneds_DistilBert
...
MODEL_CLASSES = {
    ...
    "speech_distilbert": (DistilBertConfig, gcneds_DistilBert, DistilBertTokenizer),
...
def set_seed(args): # 需要重复出现同样的模拟结果
    random.seed(args.seed)
    np.random.seed(args.seed)
    torch.manual_seed(args.seed) # 为CPU设置随机种子
    if args.n_gpu > 0:
        torch.cuda.manual_seed_all(args.seed) # GPU中设置的随机种子


def train(args, train_dataset, model, tokenizer):
    """ Train the model """
    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu) # 预训练的gpu的batch_size，根据gpu个数决定
    train_sampler = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(train_dataset) # RamdomSample：数据随机采样  DistributedSample：分布式采样器
    ...
    no_decay = ["bias", "LayerNorm.weight"] # 创建一个偏置和权重的数组进行记录
    optimizer_grouped_parameters = [ # 优化器
        {
    ...
    best_steps = 0
    model.zero_grad()
    train_iterator = trange(int(args.num_train_epochs), desc="Epoch", disable=args.local_rank not in [-1, 0]) # 加载进度条
    set_seed(args)  # Added here for reproductibility
    for _ in train_iterator:
        epoch_iterator = tqdm(train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0]) # 2622
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {
                ...
                "input_dependent": batch[4].view(-1,args.max_seq_length),
                "attention_mask": batch[5].view(-1, args.max_seq_length),
                "token_type_ids": batch[6].view(-1, args.max_seq_length)
                ...
                "labels4token": batch[7],
                "labels4sent": batch[8],
                "mat_rel_label": batch[9],
            }
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        if results[key_metric] > best_valid_f1_micro:
                            ...
                            if args.do_test:
                                results_test = evaluate(args, model, tokenizer, global_step, test=True)
                                logger.info(
...
def evaluate(args, model, tokenizer,global_step, prefix="", test=False, infer=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        with open(output_eval_file, "w") as writer:
            logger.info("***** Eval results {} *****".format(str(prefix) + "are test: " + str(test)) + "global_step: " +str(global_step))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    cached_features_file = os.path.join(
        ...
        "Cached_{}_{}_{}_{}_{}".format(
            ...
            str(args.max_seq_length), # 128
            str(args.max_mention_size), # 50
            str(task),
        ),
    )
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        label4sent_list = processor.get_labels4sent() # 句子的事件类型
        label4token_list = processor.get_labels4tokens() # 触发词的类型
    ...
    all_list_input_dependent = torch.tensor([f.list_input_dependent for f in features], dtype=torch.long)
    ...
    dataset = TensorDataset(all_example_id, all_mention_size, all_pad_token_label_id, all_list_input_ids, all_list_input_dependent, all_list_input_mask, all_list_segment_ids, all_list_label4token_ids, all_list_label4sent_ids, all_mat_rel_label_ids)
    return dataset
...
def main():
    ...
    parser.add_argument(
        "--data_dir",
        default='./Datasets/OntoEvent-Doc',
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--output_dir",
        default='./history_models/trigger-classification_ontoevent-doc_speech-bert',
        type=str,
        required=True,
    ...
    if (
        os.path.exists(args.output_dir) # 判断该文件夹是否存在
    ...
    # Prepare for task
    args.task_name = args.task_name.lower() # 将数据集的数据变为小写
    if args.task_name not in processors:
        raise ValueError("Task not found: %s" % (args.task_name))
    processor = processors[args.task_name]()
    label4token_list = processor.get_labels4tokens() # 准备tokens的trigger以及句子
    num_labels4token = len(label4token_list) # 103
    label4sent_list = processor.get_labels4sent()
    num_labels4sent = len(label4sent_list) # 返回的是None+触发词的数组的长度 101
    label4doc_list = processor.get_labels4doc()
    num_labels4doc = len(label4doc_list) # 返回的是NA+文档类型的长度 9
    ...
    config = config_class.from_pretrained(
        ...
        num_labels=num_labels4token, # 103
